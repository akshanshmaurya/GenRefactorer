...
function handleMessage(socket: WebSocket, state: ClientState, raw: string): void {
  ...
  switch (message.type) {
    case 'context.snapshot':
      state.lastContext = message.payload as WorkspaceContextSnapshot;
      break;
    case 'assistant/actionTriggered':
      handleActionTriggered(socket, state, message.payload as ActionTriggeredPayload);
      break;
    default:
...
function registerSampleActions(socket: WebSocket): void {
  const payload = {
    actions: [
      {
...
function handleActionTriggered(socket: WebSocket, state: ClientState, payload?: ActionTriggeredPayload): void {
  if (!payload?.actionId) {
    return;
  }
...
function runBannerAction(socket: WebSocket, state: ClientState): void {
...
function runFormatTask(socket: WebSocket): void {
  const payload = {
